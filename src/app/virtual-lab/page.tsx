                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                    ...
                    <div className="flex gap-3">
                        <input type="number" value={msg} onChange={e => setMsg(Number(e.target.value))}
                            className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 font-mono text-white outline-none" placeholder="Message" />
                    ...
                    {cipher && (
                        <div className="bg-secondary/10 border border-secondary/30 p-4 rounded-xl flex justify-between items-center">
                            <span className="text-slate-400 text-sm">Ciphertext c = {msg}^{keys.public_key.e} mod {keys.public_key.n}</span>
                    ...
                    {decrypted !== null && (
                        <div className="bg-success/10 border border-success/30 p-4 rounded-xl flex justify-between items-center">
                            <span className="text-slate-400 text-sm">Recovered m = {cipher.ciphertext}^{keys.private_key.d} mod {keys.public_key.n}</span>
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                    {result.result ? (
                        ...
                        <div className="bg-destructive/10 border border-destructive/30 p-6 rounded-xl text-center">
                            ...
                            <p className="text-xs text-slate-400">Quantum results are probabilistic. Run again.</p>
                        </div>
                    )}
                </motion.div>
            )}
            ...
            {result && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 gap-3">
                    <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex justify-between items-center">
                        ...
                        <div className="text-2xl font-black font-mono text-slate-300">{Math.floor(result.classical_average_steps).toLocaleString()}</div>
                    </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                    ...
                    <input type="number" value={dim} onChange={e => setDim(Number(e.target.value))}
                        className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-lg font-mono text-white focus:ring-2 focus:ring-success outline-none" />
                </div>
                <div className="space-y-1">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
                    {result.steps?.map((s: any, i: number) => (
                        <div key={i} className="bg-slate-900 border border-slate-800 p-3 rounded-lg">
                            ...
                            {s.values && <div className="text-xs text-slate-500 mt-1">{s.values}</div>}
                        </div>
                    ))}
                </motion.div>
            )}
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex items-center gap-3 md:gap-4 flex-wrap">
        <div className="flex bg-slate-900 rounded-lg md:rounded-xl p-1 border border-slate-800">
          {['Single Qubit', 'Two Qubits'].map(mode => (
            <button key={mode} onClick={() => setTwoQubitMode(mode === 'Two Qubits')}
              className={`px-3 md:px-4 py-1.5 md:py-2 text-[10px] md:text-xs font-bold rounded-lg transition-all ${(mode === 'Single Qubit') === !twoQubitMode ? 'bg-primary text-white' : 'text-slate-500 hover:text-white'}`}>
              {mode}
            </button>
          ))}
        </div>
        <button onClick={resetQubit} className="text-[10px] md:text-xs text-slate-500 hover:text-white px-2 md:px-3 py-1.5 md:py-2 rounded-lg border border-slate-800 hover:border-slate-600 transition-all">
          ↺ Reset
        </button>
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {[
                ...
                <button key={g.gate} onClick={g.action}
                  className={`bg-slate-900 border ${g.color} rounded-lg md:rounded-xl p-3 md:p-4 text-center hover:scale-[1.02] active:scale-95 transition-all`}>
          ...
          <div className="space-y-3 md:space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {twoQubitProbs.map(({ label, value, color }) => (
                <div key={label} className="bg-slate-900 border border-slate-800 rounded-lg md:rounded-xl p-2 md:p-3 text-center">
                  ...
                  <div className="w-full bg-slate-800 rounded-full h-1 md:h-1.5 mt-1 md:mt-2 overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-300" style={{ width: `${value * 100}%`, backgroundColor: color }} />
                  </div>
                </div>
              ))}
            </div>
            <div className="bg-slate-900/50 border border-slate-800 p-2 md:p-3 rounded-lg md:rounded-xl text-[10px] md:text-xs text-slate-400 text-center">
            ...
            <button onClick={() => apply('X₁', () => setTwoQubit(q => {
              ...
              className="w-full bg-slate-900 border border-slate-700 rounded-lg md:rounded-xl py-2 md:py-3 text-center hover:bg-slate-800 transition-all text-xs md:text-sm font-bold">
              X₁ (Flip first qubit)
            </button>
          </div>
          <div className="space-y-3 md:space-y-4">
            <div className="grid grid-cols-2 gap-2 md:gap-3">
              <button onClick={() => apply('CNOT', () => setTwoQubit(q => applyCNOTGate(q)))}
                ...
                <div className="text-[10px] text-slate-500 mt-1 hidden md:block">Control | Target</div>
              </button>
            ...
            <div className="bg-slate-900/50 border border-slate-800 p-2 md:p-3 rounded-lg md:rounded-xl text-[10px] md:text-xs text-slate-400 space-y-1">
      ...
      {/* History */}
      {history.length > 0 && (
        <div className="bg-slate-900/50 border border-slate-800 p-3 md:p-4 rounded-lg md:rounded-xl">
          <div className="text-[10px] md:text-xs text-slate-500 font-bold uppercase mb-1 md:mb-2">Operation History</div>
          <div className="flex flex-wrap gap-1 md:gap-2">
            {history.map((op, i) => (
              <span key={i} className={`text-[10px] font-mono px-1.5 md:px-2 py-0.5 md:py-1 rounded ${op.startsWith('Measure') ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-800 text-slate-400'}`}>
                {op}
              </span>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 md:gap-3">
        {labs.map(l => (
          <button key={l.id} onClick={() => setActive(l.id)}
            className={`glass rounded-xl md:rounded-2xl p-3 md:p-4 text-left transition-all hover:scale-[1.02] ${active === l.id ? `border-2 ${l.color} shadow-lg` : 'border border-slate-800 opacity-70 hover:opacity-100'}`}>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={active}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 min-h-[400px] md:min-h-[500px]">
            {/* Lab Panel */}
            <div className="lg:col-span-2 p-4 md:p-8 bg-slate-950/30">
            ...
            {/* Info Panel */}
            <div className="p-4 md:p-8 border-t md:border-t-0 md:border-l border-border/50 bg-slate-900/20 space-y-4 md:space-y-8 flex flex-col justify-between">
              <div className="space-y-4 md:space-y-6">
                <div>
                  <h3 className="font-bold text-primary mb-2 md:mb-3 text-sm md:text-base">About This Lab</h3>
                  <p className="text-[11px] md:text-sm text-slate-400 leading-relaxed">{lab.desc}</p>
                </div>
                {labSteps[lab.id] && (
                    ...
                    <div className="space-y-2 md:space-y-3">
                      {labSteps[lab.id].map(s => (
                        <div key={s.step} className="flex gap-2 md:gap-3">
                          ...
                          <div>
                            <p className="text-[10px] md:text-xs text-slate-300 leading-snug">{s.instruction}</p>
                            <p className="text-[8px] md:text-[10px] text-slate-500 mt-0.5 italic">{s.objective}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <button key={l.id} onClick={() => { setActive(l.id); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
              ...
              <div className="flex items-start gap-3 md:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="font-bold mb-1 group-hover:text-white transition-colors text-sm md:text-base">{l.title}</h3>
                  <p className="text-[10px] md:text-xs text-slate-400 leading-relaxed">{l.desc}</p>
                </div>
              </div>
              <div className="mt-3 md:mt-4 text-[10px] md:text-xs font-bold text-slate-500 group-hover:text-primary transition-colors flex items-center gap-1">
