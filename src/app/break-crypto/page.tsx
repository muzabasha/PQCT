  ...
  return (
    <div className="space-y-12 max-w-5xl mx-auto pb-20 px-4">
      ...
      <motion.div 
        ...
        className="bg-slate-900/60 backdrop-blur-xl border border-red-500/20 rounded-3xl p-8 lg:p-12 shadow-[0_0_50px_-12px_rgba(239,68,68,0.2)] relative overflow-hidden"
      >
        <div className="absolute top-0 right-0 p-32 bg-red-500/5 blur-[120px] rounded-full pointer-events-none"></div>

        <div className="flex flex-col sm:flex-row items-center gap-6 bg-slate-950/80 p-6 rounded-2xl border border-slate-800 relative z-10 shadow-inner">
          <div className="flex-1 space-y-2 w-full">
            <label className="text-slate-400 text-sm font-semibold uppercase tracking-wider ml-1">Target Key Size (bits)</label>
            <input 
              type="number" 
              value={keySize} 
              onChange={(e) => setKeySize(Number(e.target.value))}
              className="w-full bg-slate-900 border border-slate-700 rounded-xl px-5 py-4 text-xl font-mono text-white focus:ring-2 focus:ring-red-500 outline-none transition-all shadow-inner"
              max="32"
            />
          </div>
          <div className="flex-1 w-full pt-6 sm:pt-0">
            <button 
              onClick={runAttack}
              disabled={loading}
              className={`w-full py-4 rounded-xl font-bold transition-all shadow-lg text-lg flex justify-center items-center gap-2 ${loading ? 'bg-slate-800 text-slate-500 cursor-wait' : 'bg-gradient-to-r from-red-600 to-red-800 hover:from-red-500 hover:to-red-700 text-white shadow-red-500/30 active:scale-95'}`}
            >
        ...
        {result && (
          <div className="grid lg:grid-cols-2 gap-10 mt-12 relative z-10">
            <motion.div 
              ...
              className="bg-slate-950/80 p-8 rounded-3xl border-2 border-blue-500/30 shadow-inner space-y-6"
            >
              <h3 className="font-bold text-2xl text-blue-400 flex items-center gap-3">
                <span className="text-3xl">🛡️</span> 1. Classical Target
              </h3>
              <p className="text-slate-400 text-sm">We generated a vulnerable RSA public key.</p>
              
              <div className="space-y-4">
                <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                  <div className="text-xs uppercase tracking-wider text-slate-500 font-bold mb-1">Public Modulus (N)</div>
                  <div className="font-mono text-2xl text-blue-300 break-all">{result.rsa.public_key.n}</div>
                </div>
                <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                  <div className="text-xs uppercase tracking-wider text-slate-500 font-bold mb-1">Hidden Primes (p, q)</div>
                  <div className="font-mono text-xl text-slate-400">{result.rsa.details.p}, {result.rsa.details.q}</div>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              <p className="text-slate-400 text-sm relative z-10">Shor's algorithm intercepted and factored N.</p>
              ...
              <div className="h-full relative z-10">
                {result.shor.result ? (
                  ...
                  <motion.div 
                    initial={{ scale: 0.9 }} animate={{ scale: 1 }}
                    className="bg-slate-900 p-6 rounded-xl border border-slate-700 text-center h-full flex flex-col justify-center"
                  >
                    <div className="text-slate-400 font-bold mb-2">Probabilistic Failure</div>
