...
export function Math({ math, block = false }: MathProps) {
  if (block) {
    return <div className="my-4 p-4 bg-white rounded-xl border border-slate-200 shadow-inner overflow-x-auto"><BlockMath math={math} /></div>;
  }
...
export function MathSteps({ steps }: { steps: { step: string; formula: string; values: string; message?: string }[] }) {
  return (
    <div className="space-y-4">
      {steps.map((step, i) => (
        <div key={i} className="p-4 bg-white rounded-lg border border-slate-200 flex flex-col md:flex-row md:items-center gap-4">
          ...
          <div className="flex-1">
            ...
            {step.message ? (
               <p className="text-slate-600 mt-1">{step.message}</p>
            ) : (
               <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div>
                   <div className="text-xs text-slate-600 mb-1">Formula</div>
                   <Math math={step.formula} />
                 </div>
                 <div>
                   <div className="text-xs text-slate-600 mb-1">Values</div>
